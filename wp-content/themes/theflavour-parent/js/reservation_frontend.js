...
jQuery(document).ready(function(){
    ...
    $('.tfuse_rf_post_datepicker_in').datepicker({
        ...
        {
            inst.dpDiv.css({marginTop: -input.offsetHeight+52 + 'px'});
        }
...
function showErrorMessage(textMessage){
    var $=jQuery;
    _message= _form.closest('.container').find('#form_messages').html('<h2>'+textMessage+'</h2>').addClass('error_submiting_form').show();
    _form.hide();
}
...
function showSuccessMessage(textMessage){
    var $=jQuery;
    _message=  _form.closest('.container').find('#form_messages').html('<h2>'+textMessage+'</h2>').addClass('success_submited_form').show();
    _form.hide();
}
function check_fields() {
    ...
    if (_form.find('.'+_theme_index + '_email').length>0) {
		...
		if( _captcha_resp && _return_val )
        {
            ...
            _form.closest('.container').find('#header_message').hide();
            _form.find('#sending').css('display','inline-block');
            var form_section = _form.parent('.reservation-container').parent('section');
            var form_height = form_section.outerHeight();
            form_section.css('height', form_height);
            form_section.find('#form_messages').css('margin-top', (form_height-150)/2);
		}
